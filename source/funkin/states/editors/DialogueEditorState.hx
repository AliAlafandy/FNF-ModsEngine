...
import funkin.data.objects.TypedAlphabet;

import funkin.data.cutscenes.DialogueBoxMod;
import funkin.data.cutscenes.DialogueCharacter;
...
class DialogueEditorState extends MusicBeatState
{
	...
	override function create() {
		...
		daText = new TypedAlphabet(DialogueBoxMod.DEFAULT_TEXT_X, DialogueBoxMod.DEFAULT_TEXT_Y, DEFAULT_TEXT);
		daText.setScale(0.7);
		add(daText);
		changeText();
	...
	function updateTextBox() {
		...
		DialogueBoxMod.updateBoxOffsets(box);
	}
	...
	function reloadCharacter() {
		...
		character.x = DialogueBoxMod.LEFT_CHAR_X;
		character.y = DialogueBoxMod.DEFAULT_CHAR_Y;

		switch(character.jsonFile.dialogue_pos) {
			case 'right':
				character.x = FlxG.width - character.width + DialogueBoxMod.RIGHT_CHAR_X;
	...
	function reloadText(skipDialogue:Bool) {
		...
		daText.y = DialogueBoxMod.DEFAULT_TEXT_Y;
		if(daText.rows > 2) daText.y -= DialogueBoxMod.LONG_TEXT_ADD;

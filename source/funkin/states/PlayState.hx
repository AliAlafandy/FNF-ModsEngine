...
import funkin.data.backend.Song;
import funkin.data.backend.Section;
import funkin.data.backend.Rating;
...
import funkin.data.cutscenes.DialogueBoxMod;
...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		#if (LUA_ALLOWED || HSCRIPT_ALLOWED)
		luaDebugGroup = new FlxTypedGroup<funkin.data.scripts.DebugLuaText>();
		...
		lyricsBG = new FlxSprite();
		lyricsBG.makeGraphic(1, 1, FlxColor.BLACK);
		lyricsBG.alpha = 0.6;
		lyricsBG.cameras = [camHUD];
		lyricsBG.visible = false;
		uiGroup.add(lyricsBG);

		lyricsText = new FlxText(0, 0, 0, "");
		lyricsText.setFormat(Paths.font("vcr.ttf"), 32, FlxColor.WHITE, CENTER);
		lyricsText.cameras = [camHUD];
		lyricsText.visible = false;
		uiGroup.add(lyricsText);
	...
	public function addTextToDebug(text:String, color:FlxColor) {
		...
		luaDebugGroup.forEachAlive(function(spr:funkin.data.scripts.DebugLuaText) {
	...
	public var psychDialogue:DialogueBoxMod;
	//You don't have to add a song, just saying. You can just do "startDialogue(DialogueBoxMod.parseDialogue(Paths.json(songName + '/dialogue')))" and it should load dialogue.json
	public function startDialogue(dialogueFile:DialogueFile, ?song:String = null):Void
	{
		...
		if(dialogueFile.dialogue.length > 0) {
			inCutscene = true;
			psychDialogue = new DialogueBoxMod(dialogueFile, song);
	...
	override function destroy() {
		...
		funkin.data.backend.NoteTypesConfig.clearNoteTypesData();
		instance = null;
		super.destroy();
	}
